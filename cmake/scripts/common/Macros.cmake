  ...
  foreach(pat ${pattern})
    ...
    foreach(filename ${filenames})
      ...
      foreach(dir ${fstrings})
        ...
        if (dest AND (src MATCHES "\\/\\*$") AND (dest MATCHES "\\/\\*$"))
        ...
        foreach(file ${files})
          if (srcPath AND destPath)
            STRING(REPLACE ${srcPath} ${destPath} fileDest ${file})
            get_filename_component(fileDest "${fileDest}" PATH)
          else()
            set(fileDest ${dest})
          endif()
          if(arg_NO_INSTALL)
            copy_file_to_buildtree(${CMAKE_SOURCE_DIR}/${file} DIRECTORY ${fileDest} NO_INSTALL)
          else()
            copy_file_to_buildtree(${CMAKE_SOURCE_DIR}/${file} DIRECTORY ${fileDest})
          endif()
        endforeach()
      endforeach()
    endforeach()
